const {subHours, endOfDay, startOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear } = require('date-fns');
...
class TaskControllers {
    ...
    async today(req, res){
        ...
        await TaskModel
        .find({'when': {'$gte': startOfDay(current), '$lt': endOfDay(current) }})
        .sort('when')
        .then(response => {
            return res.status(200).json(response);
        })
    ...
    async week(req, res){
        const current = new Date();
        await TaskModel
        .find({'when': {'$gte': startOfWeek(current), '$lt': endOfWeek(current) }})
        .sort('when')
        .then(response => {
            return res.status(200).json(response);
        })
    ...
    async month(req, res){
        const current = new Date();
        await TaskModel
        .find({'when': {'$gte': startOfMonth(current), '$lt': endOfMonth(current) }})
        .sort('when')
        .then(response => {
            return res.status(200).json(response);
        })
    ...
    async year(req, res){
        const current = new Date();
        await TaskModel
        .find({'when': {'$gte': startOfYear(current), '$lt': endOfYear(current) }})
        .sort('when')
        .then(response => {
            return res.status(200).json(response);
        })
